...
class Controller:
    ...
    #
    # ***Note that the Maestro itself is configured to limit the range of
    # servo travel which has precedence over these values.
    # Use the Maestro Control Center to configure
    ...
    def setSpeed(self, chan, speed):
        ...
        # Send Pololu intro, device number, command, channel, speed lsb, speed
        # msb
    ...
    def setAccel(self, chan, accel):
        ...
        # Send Pololu intro, device number, command, channel, accel lsb, accel
        # msb
        cmd = self.PololuCmd + chr(0x09) + chr(chan) + chr(lsb) + chr(msb)
        self.usb.write(cmd)

    # Get the current position of the device on the specified channel
    # The result is returned in a measure of quarter-microseconds,
    # which mirrors the Target parameter of setTarget.
    # This is not reading the true servo position, but the last target
    # position sent to the servo. If the Speed is set to below the top
    # speed of the servo, then the position result will align well with
    #  the acutal servo position, assuming it is not stalled or slowed.
    ...
    # Test to see if a servo has reached its target position.  This only
    # provides useful results if the Speed parameter is set slower than
    # the maximum speed ofthe servo.
    # ***Note if target position goes outside of Maestro's allowable range
    # for the channel, then the target can never be reached, so it will
    # appear to allows be moving to the target.  See setRange comment.
    ...
    # Speed and/or Acceleration have been set on one or more of the channels.
    ...
    # Scripts can have multiple subroutines, which get numbered sequentially
    # from 0 on up.
    # Code your Maestro subroutine to either infinitely loop, or just end
    # (return is not valid).
